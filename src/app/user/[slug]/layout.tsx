    ...
    <div className="flex gap-5 items-start">
      <div className="w-1/4 bg-slate-200 dark:bg-slate-800 rounded-md p-5 flex flex-col h-auto">
        <div className="flex flex-col gap-2 pb-5 mb-5 border-b-2 border-slate-500 items-start">
          ...
          {profile.username === profile.id && (
            <InfoLabel>
              Это имя пользователя было сгенерировано автоматически. Измените его в разделе
              «Редактировать».
            </InfoLabel>
          )}
        ...
        <div className="flex flex-col gap-2 items-start">
          <Label size="small">Оцененные матчи: {profile.comments_matches.length}</Label>
          <Label size="small">Оцененные рестлеры: {profile.comments_wrestlers.length}</Label>
          <Label size="small">Оцененные шоу: {profile.comments_shows.length}</Label>
          <Label size="small">Оцененные команды: {profile.comments_teams.length}</Label>
          <Label size="small">Оцененные турниры: {profile.comments_tournaments.length}</Label>
      ...
      <div className="flex-1">
        <div className="flex justify-center gap-2 mb-5">
          ...
          <SectionButton link={`/user/${params.slug}/wrestlers`}>Рестлеры</SectionButton>
          ...
          <SectionButton link={`/user/${params.slug}/tournaments`}>Турниры</SectionButton>
          <SectionButton link={`/user/${params.slug}/titles`}>Титулы</SectionButton>
          {user?.id === profile.id && (
            <SectionButton link={`/user/${params.slug}/change`}>Редактировать</SectionButton>
          )}
        </div>
