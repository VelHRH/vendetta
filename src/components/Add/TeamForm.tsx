...
const TeamForm = ({
  ...
  const [creationDate, setCreationDate] = useState<string>(team?.creation_date || '');
  const [disbandDate, setDisbandDate] = useState<string>(team?.disband_date || '');
  ...
  const [wrestlers, setWrestlers] = useState<Database['public']['Tables']['wrestlers']['Row'][]>(
    [],
  );
  const [titles, setTitles] = useState<Database['public']['Tables']['titles']['Row'][]>([]);
  ...
  const uploadImage = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      let { error: uploadError } = await supabase.storage.from('wrestlers').upload(filePath, file, {
        cacheControl: '3600',
        upsert: false,
      });
    ...
    mutationFn: async () => {
      const payload: CreateTeamPayload = {
        ...
        reigns: reigns
          ...
          .map(reign =>
            reign.filter(champion => champion.wrestlerId !== 0 || champion.titleId !== 0),
          ),
  ...
  const { mutate: updateTeam, isLoading: isLoadingUpdate } = useMutation({
    ...
    onError: err => {
      ...
      toast({
        title: 'There was an error',
        description: 'Could not update the team',
        variant: 'destructive',
      });
    },
    onSuccess: data => {
  ...
  return (
    <div className="flex flex-col items-center gap-10 w-full">
      <Label className="font-bold">{team ? 'Editing team...' : 'Creating new team...'}</Label>
                  ...
                  setValue={newVal =>
                    setReigns(prev => {
                      ...
                      newReigns[index][index2].wrestlerId = parseFloat(
                        current_participants.find(p => p.wrestlerCurName === newVal)!.wrestlerId,
                      );
                      return newReigns;
                    })
                  }
...
const reignsSetter = (reigns: Database['public']['Tables']['reigns']['Row'][]) => {
  const reignsFiltered = [...reigns].filter(
    (reign, index, self) =>
      index ===
      self.findIndex(
        r => r.start === reign.start && r.end === reign.end && r.team_id === reign.team_id,
      ),
  );
  const newReigns: {
    wrestlerName: string;
